%% This file solves the OLG model for the other scenario.
%% Scenario: Basic income program replaces public pension system and
%% is financed by an increase in the consumption tax rate.
clear all;
...
% Production function
Params.alpha = 0.42; % Share of capital　自分で設定。
...
% Taxes
Params.tau_c = 0.6; % Consumption tax rate
...
Params.replacement = 0.40; % Replacement ratio of transfer to labor income
...
Params.tau_y = 0.198; % This is computed under the public pension system of Policy 1.
Params.kshare = 0.9; % Share of capital in assets. 自分で追加。0.9でやると、kshareが1を超える。
...
GEPriceParamNames={'r','bi','G','tau_c', 'kshare'}; 
...
writematrix(AgeConditionalStats.Asset.Mean,'./BI_consumption/LifeCycleProfiles_Asset_BI_consumption.txt'); 
writematrix(AgeConditionalStats.H.Mean,'./BI_consumption/LifeCycleProfiles_H_BI_consumption.txt'); 
writematrix(AgeConditionalStats.Consumption.Mean,'./BI_consumption/LifeCycleProfiles_Consumption_BI_consumption.txt'); 
writematrix((AgeConditionalStats.IncomeTaxRevenue.Mean/Params.tau_y - AgeConditionalStats.Consumption.Mean), './BI_consumption/LifeCycleProfiles_SavingRate_BI_consumption.txt'); 
...
xlabel('Real age')
writematrix(all_age_bi./AgeConditionalStats.IncomeTaxRevenue.Mean,'./BI_consumption/LifeCycleProfiles_BItoincometax_BI_consumption.txt'); % 自分で追加。
...
fprintf('Consumption tax rate: tau_c=%8.3f \n', Params.tau_c)
...
writematrix(Params.tau_y, './BI_consumption/Income_tax_rate_BI_consumption.txt')
writematrix(Params.tau_c, './BI_consumption/Consumption_tax_rate_BI_consumption.txt')
writematrix(AggVars.H.Mean, './BI_consumption/Average_time_spent_at_work_BI_consumption.txt')
writematrix(Params.kshare, './BI_consumption/kshare_BI_consumption.txt')
writematrix(AggVars.Asset.Mean, './BI_consumption/Average_Asset_BI_consumption.txt')
writematrix(K, './BI_consumption/Average_K_BI_consumption.txt')
writematrix(Y, './BI_consumption/Average_Y_BI_consumption.txt')
writematrix(AggVars.Consumption.Mean, './BI_consumption/Average_C_BI_consumption.txt')
...
writematrix(LorenzCurve.Asset, './BI_consumption/Lorenz_Asset_BI_consumption.txt');
writematrix(LorenzCurve.Consumption, './BI_consumption/Lorenz_Consumption_BI_consumption.txt');
...
writematrix(Gini_Asset,'./BI_consumption/Gini_Asset_BI_consumption.txt'); 
Gini_Consumption = Gini_from_LorenzCurve(LorenzCurve.Consumption); 
writematrix(Gini_Consumption,'./BI_consumption/Gini_Consumption_BI_consumption.txt'); 
...
writematrix(AvWorkTime,'./BI_consumption/Average_time_at_work_BI_consumption.txt'); 
fprintf('Basic Income per person: =%8.4f \n', Params.bi);
writematrix(Params.bi,'./BI_consumption/Pension_perperson_BI_consumption.txt'); 
